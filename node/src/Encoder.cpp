...
void Encoder::pulse_helper(int gpio, int level, uint32_t tick, void* user)
{
...
: m_chan_a(chan_a), m_chan_b(chan_b), m_lvlA(0), m_lvlB(0), m_lastGpio(chan_a.number()),
  m_relcount(0) {
    m_chan_a.onChange(Encoder::pulse_helper, this);
    m_chan_b.onChange(Encoder::pulse_helper, this);
}
